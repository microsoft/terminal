...
void CascadiaSettings::LogSettingChanges(bool isJsonLoad) const
{
    ...
    _baseWindowSettings->LogSettingChanges(changes, globalContext);
    for (const auto& [name, window] : _windows)
    {
        winrt::get_self<implementation::WindowSettings>(window)->LogSettingChanges(changes, globalContext);
    }
