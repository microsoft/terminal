...
{
    static ScopedResourceLoader foo = GetLibraryResourceLoader().WithQualifier(L"language", L"en-US");
    return foo;
}
...
#undef ON_ALL_ACTIONS_WITH_ARGS
    ...
    winrt::hstring ActionAndArgs::_generateName(bool localized) const
    {
        ...
        static const auto GeneratedActionNames = []() {
            return std::unordered_map<ShortcutAction, std::wstring_view>{
        ...
        if (_Args)
        {
            auto nameFromArgs = _Args.GenerateName(localized);
        ...
        if (found != GeneratedActionNames.end() && !found->second.empty())
        {
            ...
            return loader.GetLocalizedString(found->second);
        }
        return winrt::hstring{};
    }
    winrt::hstring ActionAndArgs::GenerateName() const { return _generateName(true); }
    winrt::hstring ActionAndArgs::GenerateLanguageNeutralName() const { return _generateName(false); }
