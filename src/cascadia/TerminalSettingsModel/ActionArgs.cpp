...
namespace winrt::Microsoft::Terminal::Settings::Model::implementation
{
    winrt::hstring NewTerminalArgs::GenerateName() const
    {
        ...
        if (!Profile().empty())
        {
            fmt::format_to(it, FMT_COMPILE(L"profile: {}, "), Profile());
        }
        else if (ProfileIndex())
        {
            fmt::format_to(it, FMT_COMPILE(L"profile index: {}, "), ProfileIndex().Value());
        }

        if (!Commandline().empty())
        {
            fmt::format_to(it, FMT_COMPILE(L"commandline: {}, "), Commandline());
        }

        if (!StartingDirectory().empty())
        {
            fmt::format_to(it, FMT_COMPILE(L"directory: {}, "), StartingDirectory());
        }

        if (!TabTitle().empty())
        {
            fmt::format_to(it, FMT_COMPILE(L"title: {}, "), TabTitle());
        }

        if (TabColor())
        {
            const til::color tabColor{ TabColor().Value() };
            fmt::format_to(it, FMT_COMPILE(L"tabColor: {}, "), tabColor.ToHexString(true));
        }
        if (!ColorScheme().empty())
        {
            fmt::format_to(it, FMT_COMPILE(L"colorScheme: {}, "), ColorScheme());
        }
        ...
        if (SuppressApplicationTitle())
        {
            ...
            {
                fmt::format_to(it, FMT_COMPILE(L"suppress application title, "));
            }
            else
            {
                fmt::format_to(it, FMT_COMPILE(L"use application title, "));
            }
        }

        if (Elevate())
        {
            fmt::format_to(it, FMT_COMPILE(L"elevate: {}, "), Elevate().Value());
        }
