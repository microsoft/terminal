...
bool Pane::_Resize(const ResizeDirection& direction)
{
    ...
    const Size actualSize{ til::narrow_cast<float>(_root.ActualWidth()),
                           til::narrow_cast<float>(_root.ActualHeight()) };
...
std::shared_ptr<Pane> Pane::NavigateDirection(const std::shared_ptr<Pane> sourcePane, const FocusDirection& direction, const std::vector<uint32_t>& mruPanes)
{
    ...
    const auto scaleX = _root.ActualWidth() > 0 ? til::narrow_cast<float>(_root.ActualWidth()) : 1.f;
    const auto scaleY = _root.ActualHeight() > 0 ? til::narrow_cast<float>(_root.ActualHeight()) : 1.f;
