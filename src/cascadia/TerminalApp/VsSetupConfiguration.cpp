...
#ifdef _DEBUG
void TerminalApp::VsSetupConfiguration::DebugOutputProperties(std::wstring_view header, const ComPtrPropertyStore pProps)
{
    OutputDebugString(L"<=========== ");
    OutputDebugString(header.data());
    OutputDebugString(L" ===========>\n");

    LPSAFEARRAY psaNames;
    THROW_IF_FAILED(pProps->GetNames(&psaNames));

    auto cleanup = wil::scope_exit([&] {
        SafeArrayDestroy(psaNames);
    });

    THROW_IF_FAILED(SafeArrayLock(psaNames));

    auto rgbNames = reinterpret_cast<BSTR*>(psaNames->pvData);
    auto cNames = psaNames->rgsabound[0].cElements;

    if (0 == cNames)
        return;

    std::vector<wil::unique_bstr> names(rgbNames, rgbNames + cNames);
    for (const auto& n : names)
    {
        std::wstring propName{ n.get() };
        wil::unique_variant var;

        try
        {
            THROW_IF_FAILED(pProps->GetValue(propName.c_str(), &var));

            std::wstring msg{ L"PROPERTY: " + propName + L"\n" };
            ...
            {
            case VT_BOOL:
                val.append(var.boolVal == VARIANT_TRUE ? L"TRUE" : L"FALSE");
                break;
            case VT_BSTR:
                val.append(var.bstrVal);
                break;
            default:
                val.append(L"OTHER TYPE = " + var.vt);
            };
