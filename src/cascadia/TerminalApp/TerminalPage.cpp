...
namespace winrt::TerminalApp::implementation
{
    ...
    void TerminalPage::Create()
    {
        ...
        CommandPalette().SetTabs(_tabs);
        CommandPalette().SetMRUTabs(_mruTabs);
    ...
    void TerminalPage::_SelectNextTab(const bool bMoveRight)
    {
        ...
        if (useTabSwitcher)
        {
            ...
            CommandPalette().EnableTabSwitcherMode(newTabIndex, tabSwitchMode);
    ...
    // Return Value:
    // - <none>
    winrt::fire_and_forget TerminalPage::_SetFocusedTab(const TerminalApp::TabBase& tab)
    {
        auto weakThis{ get_weak() };
        auto weakTab{ _GetTerminalTabImpl(tab)->get_weak() };

        co_await winrt::resume_foreground(_tabView.Dispatcher());

        auto page{ weakThis.get() };
        auto tabToFocus{ weakTab.get() };
        if (page && tabToFocus)
        {
            auto tabViewItem{ tabToFocus->TabViewItem() };

            // Select tab item only if it is still present in the TabView
            uint32_t tabIndex;
