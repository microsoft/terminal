...
namespace winrt::TerminalApp::implementation
{
    ...
    bool TmuxControl::ActiveTabIsTmuxControl()
    {
        auto activeTab = _page._GetFocusedTab();
        auto tmuxActiveTab = _GetTab(_activeWindowId);
        if (activeTab && tmuxActiveTab)
        {
            if (activeTab.TabViewIndex() == tmuxActiveTab.TabViewIndex())
            {
                return true;
            }
        }
        if (activeTab && _controlTab)
        {
            if (activeTab.TabViewIndex() == _controlTab.TabViewIndex())
            {
                return true;
            }
        }
    ...
    void TmuxControl::SplitActivePane(SplitDirection direction)
    {
        ...
        auto activeTab = _GetTab(_activeWindowId);
        const auto realSplitType = activeTab.try_as<TerminalTab>()->PreCalculateCanSplit(direction, 0.5f, availableSpace);
    ...
    void TmuxControl::_OpenNewTerminalViaDropdown()
    {
        ...
        if (altPressed)
        {
            ...
            if (ActiveTabIsTmuxControl())
            {
                SplitActivePane(SplitDirection::Automatic);
            }
        }
        else
        {
    ...
    {
        auto rootPane = _NewPane(windowId, paneId);
    ...
    void TmuxControl::_SplitPaneFinalize(int windowId, int newPaneId)
    {
        ...
        _attachedPanes.insert({_activePaneId, {windowId, _activePaneId, origin->GetTerminalControl(), true}});

        newGuy->GetTerminalControl().Focus(FocusState::Programmatic);
        _splittingPane.first = -1;
    }
    ...
    {
        if (_splittingPane.first != -1)
        {
            return;
        }

        _splittingPane = {_activePaneId, direction};
        auto cmd = std::make_unique<SplitPane>();
        cmd->direction = direction;
        cmd->paneId = _splittingPane.first;
        _SendCommand(std::move(cmd));
        _ScheduleCommand();
    }
