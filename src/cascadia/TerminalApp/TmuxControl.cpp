...
namespace winrt::TerminalApp::implementation
{
    ...
    void TmuxControl::SplitPane(const winrt::com_ptr<Tab>& tab, SplitDirection direction)
    {
        ...
        const auto realSplitType = tab.try_as<Tab>()->PreCalculateCanSplit(direction, 0.5f, availableSpace);
        if (!realSplitType)
        {
            return;
        }
    ...
    TerminalApp::Tab TmuxControl::_GetTab(int windowId) const
    {
    ...
    void TmuxControl::_RenameWindow(int windowId, const std::wstring& name)
    {
        ...
        tab.try_as<Tab>()->SetTabText(winrt::hstring{ name });
    }
    ...
    void TmuxControl::_NewWindowFinalize(int windowId, int paneId, const std::wstring& windowName)
    {
        ...
        winrt::get_self<Tab>(tab)->CloseRequested([this, windowId](auto&&, auto&&) {
            _KillWindow(windowId);
        });

        tab.try_as<Tab>()->SetTabText(winrt::hstring{ windowName });
    ...
    void TmuxControl::_SplitPaneFinalize(int windowId, int newPaneId)
    {
        ...
        auto activePane = tab.try_as<Tab>()->GetActivePane();
    ...
    bool TmuxControl::_SyncWindowState(std::vector<TmuxWindow> windows)
    {
        for (auto& w : windows)
        {
            ...
            winrt::get_self<Tab>(tab)->SetTabText(winrt::hstring{ w.name });
