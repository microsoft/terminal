...
void ConptyRoundtripTests::TestWrappingALongString()
{
    ...
    const auto charsToWrite = til::narrow_cast<til::CoordType>(TestUtils::Test100CharsString.size());
...
void ConptyRoundtripTests::TestCursorInDeferredEOLPositionOnNewLineWithSpaces()
{
    ...
    hostSm.ProcessString(std::wstring(til::narrow_cast<size_t>(TerminalViewHeight), L'\n'));
    // fill width-1 with "A", then add one space and another character..
    hostSm.ProcessString(std::wstring(til::narrow_cast<size_t>(TerminalViewWidth) - 1, L'A') + L" B");
    ...
    auto verifyBuffer = [&](const TextBuffer& tb, til::CoordType bottomRow) {
        ...
        auto expectedStringSecondToLastRow{ std::wstring(til::narrow_cast<size_t>(tb.GetSize().Width()) - 1, L'A') + L" " };
