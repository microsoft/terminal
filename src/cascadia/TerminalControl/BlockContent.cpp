...
{
    BlockContent::BlockContent(Control::IControlSettings settings,
                               TerminalConnection::ITerminalConnection connection) :
        _connection{ connection },
        _desiredFont{ DEFAULT_FONT_FACE, 0, DEFAULT_FONT_WEIGHT, DEFAULT_FONT_SIZE, CP_UTF8 },
        _actualFont{ DEFAULT_FONT_FACE, 0, DEFAULT_FONT_WEIGHT, { 0, DEFAULT_FONT_SIZE }, CP_UTF8, false }
    {
        _settings = winrt::make_self<implementation::ControlSettings>(settings, nullptr);
    ...
    void BlockContent::_setupDispatcherAndCallbacks()
    {
        ...
        _updateScrollBar = std::make_shared<ThrottledFuncTrailing<Control::ScrollPositionChangedArgs>>(
            ...
            [weakThis = get_weak()](const auto& update) {
                if (auto content{ weakThis.get() }; !content->_IsClosing())
                {
                    content->_ScrollPositionChangedHandlers(*content, update);
                }
            });
    }
    ...
    bool BlockContent::Initialize(const double actualWidth,
    ...
    // Return Value:
    // - <none>
    void BlockContent::EnablePainting()
    {
        if (_initializedTerminal)
        {
            _renderer->EnablePainting();
    ...
    // Return Value:
    // - <none>
    void BlockContent::_sendInputToConnection(std::wstring_view wstr)
    {
        // if (_isReadOnly)
        // {
    ...
    bool BlockContent::SendCharEvent(const wchar_t ch,
                                     const WORD scanCode,
    ...
    void BlockContent::_handleControlC()
    {
    ...
    bool BlockContent::TrySendKeyEvent(const WORD vkey,
                                       const WORD scanCode,
    ...
    void BlockContent::UpdateSettings(const IControlSettings& settings /*, const IControlAppearance& newAppearance*/)
    {
        _settings = winrt::make_self<implementation::ControlSettings>(settings, nullptr);
    ...
    void BlockContent::_updateAntiAliasingMode()
    {
    ...
    void BlockContent::_updateFont(const bool /*initialUpdate*/)
    {
    ...
    bool BlockContent::_setFontSizeUnderLock(float fontSize)
    {
    ...
    // Return Value:
    // - <none>
    void BlockContent::_refreshSizeUnderLock()
    {
        // TODO!!
        if (_IsClosing())
        {
    ...
    void BlockContent::SizeChanged(const double width,
                                   const double height)
    {
        SizeOrScaleChanged(width, height, _compositionScale);
    }

    void BlockContent::ScaleChanged(const double scale)
    {
        if (!_renderEngine)
        {
            return;
    ...
    void BlockContent::SizeOrScaleChanged(const double width,
                                          const double height,
                                          const double scale)
    {
    ...
    winrt::Windows::Foundation::Size BlockContent::FontSize() const noexcept
    {
    ...
    TerminalConnection::ConnectionState BlockContent::ConnectionState() const
    {
    ...
    til::color BlockContent::BackgroundColor() const
    {
    ...
    int BlockContent::ViewHeight() const
    {
        // TODO!!
    ...
    // Return Value:
    // - The height of the terminal in lines of text
    int BlockContent::BufferHeight() const
    {
        // TODO!!
    ...
    void BlockContent::_renderEngineSwapChainChanged(const HANDLE handle)
    {
    ...
    bool BlockContent::CursorOn() const
    {
        return _terminal->IsCursorOn();
    }

    void BlockContent::CursorOn(const bool isCursorOn)
    {
    ...
    //     _RaiseNoticeHandlers(*this, std::move(noticeArgs));
    // }
    void BlockContent::_connectionOutputHandler(const hstring& hstr)
    {
        try
        {
    ...
    uint64_t BlockContent::SwapChainHandle() const
    {
        // This is called by:
