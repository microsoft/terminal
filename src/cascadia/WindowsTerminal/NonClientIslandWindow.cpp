...
til::rect NonClientIslandWindow::_GetDragAreaRect() const noexcept
{
    if (_dragBar && _dragBar.Visibility() == Visibility::Visible)
    {
        ...
        const auto x = til::narrow_cast<til::CoordType>(clientDragBarRect.X * scale);

        return {
            x,
            til::narrow_cast<til::CoordType>(clientDragBarRect.Y * scale),
            til::narrow_cast<til::CoordType>((clientDragBarRect.Width + clientDragBarRect.X) * scale) - x,
            til::narrow_cast<til::CoordType>((clientDragBarRect.Height + clientDragBarRect.Y) * scale),
        };
    }
