...
HMENU NotificationIcon::_CreateContextMenu(const IVectorView<winrt::Microsoft::Terminal::Remoting::PeasantInfo>& peasants)
{
    ...
    if (hMenu)
    {
        ...
        AppendMenu(hMenu, MF_STRING, wil::safe_cast<UINT_PTR>(NotificationIconMenuItemAction::FocusTerminal), RS_(L"NotificationIconFocusTerminal").c_str());
        ...
        if (auto submenu = CreatePopupMenu())
        {
            for (const auto& p : peasants)
            {
                ...
                AppendMenu(submenu, MF_STRING, wil::safe_cast<UINT_PTR>(p.Id), displayText.str().c_str());
            }
...
void NotificationIcon::MenuItemSelected(const HMENU menu, const UINT menuItemIndex)
{
    ...
    if (mi.dwMenuData)
    {
        if (wil::safe_cast<NotificationIconMenuItemAction>(mi.dwMenuData) == NotificationIconMenuItemAction::SummonWindow)
        {
    ...
    const auto action = wil::safe_cast<NotificationIconMenuItemAction>(GetMenuItemID(menu, menuItemIndex));
    switch (action)
    {
