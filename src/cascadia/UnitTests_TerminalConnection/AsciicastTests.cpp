...
namespace TerminalConnectionUnitTests
{
    ...
    {
        Log::Comment(L"Verify _WriteHeader produces valid asciicast v2 header JSON.");
        ...
        // Verify the line starts with {"version": 2
        VERIFY_IS_TRUE(line.find("{\"version\": 2") == 0);
        VERIFY_IS_TRUE(line.find("\"width\": 80") != std::string::npos);
        VERIFY_IS_TRUE(line.find("\"height\": 24") != std::string::npos);
    ...
    {
        Log::Comment(L"Verify _WriteEvent produces a correctly formatted event array.");
        ...
        recorder._startTime = std::chrono::high_resolution_clock::now();
