...
[[nodiscard]] std::wstring ConvertToW(const UINT codePage, const std::string_view source)
{
    ...
    SetLastError(0);
    const int iTarget = MultiByteToWideChar(codePage, 0, source.data(), iSource, nullptr, 0);
...
#pragma prefast(suppress: __WARNING_W2A_BEST_FIT, "WC_NO_BEST_FIT_CHARS doesn't work in many codepages. Retain old behavior.")
    // clang-format on
    const int iTarget = WideCharToMultiByte(codepage, 0, source.data(), iSource, nullptr, 0, nullptr, nullptr);
