...
func run() error {
	...
	for scanner.Scan() {
		...
		if firstLine {
			...
			_, _ = fmt.Fprintf(&testString, "// Generated by CodepointWidthDetector_gen.go\n")
			_, _ = fmt.Fprintf(&testString, "// on %s, from Unicode %s\n", time.Now().UTC().Format(time.RFC3339), m[1])

			testString.WriteString("struct GraphemeBreakTest\n")
