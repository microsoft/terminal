...
namespace Microsoft::Console::VirtualTerminal
{
                        ...
                        const VTParameters options) noexcept
    {
        ...
        if (_numSavedAttrs < wil::safe_cast<int>(_storedSgrAttributes.size()))
        {
            _numSavedAttrs++;
        }

        _storedSgrAttributes.at(_nextPushIndex) = { currentAttributes, validParts };
        _nextPushIndex = (_nextPushIndex + 1) % wil::safe_cast<int>(_storedSgrAttributes.size());
    }
    ...
    const TextAttribute SgrStack::Pop(const TextAttribute& currentAttributes) noexcept
    {
        if (_numSavedAttrs > 0)
        {
            ...
            if (_nextPushIndex == 0)
            {
                _nextPushIndex = wil::safe_cast<int>(_storedSgrAttributes.size() - 1);
            }
            else
            {
                _nextPushIndex--;
