...
Popup::Popup(SCREEN_INFORMATION& screenInfo, const til::point proposedSize) :
...
til::point Popup::_CalculateSize(const SCREEN_INFORMATION& screenInfo, const til::point proposedSize)
{
    ...
    size.X = std::min(size.X, viewportSize.width);
    size.Y = std::min(size.Y, viewportSize.height);
...
til::point Popup::_CalculateOrigin(const SCREEN_INFORMATION& screenInfo, const til::point size)
{
