...
[[nodiscard]] HRESULT StartNullConsole(const ConsoleArguments* const args)
{
    ...
    fakeConnectInfo.TitleLength = til::narrow_cast<DWORD>(fakeTitle.size() * sizeof(wchar_t)); // bytes, not wchars
    wcscpy_s(fakeConnectInfo.AppName, fakeTitle.data());
    fakeConnectInfo.AppNameLength = til::narrow_cast<DWORD>(fakeTitle.size() * sizeof(wchar_t)); // bytes, not wchars
