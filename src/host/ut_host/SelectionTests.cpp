...
class SelectionTests
{
    ...
    void VerifyGetSelectionRects_BoxMode()
    {
        ...
        {
            for (auto iRect = 0; iRect < wil::safe_cast<int>(selectionRects.size()); iRect++)
            {
    ...
    void VerifyGetSelectionRects_LineMode()
    {
        ...
        if (VERIFY_ARE_EQUAL(cRectanglesExpected, selectionRects.size()))
        {
            ...
            else
            {
                ...
                UINT iFirst = 0;
                auto iLast = wil::safe_cast<UINT>(selectionRects.size() - 1u);
...
class SelectionInputTests
{
    ...
    TEST_METHOD(TestWordByWordPrevious)
    {
        ...
        const til::point right{ wil::safe_cast<til::CoordType>(text.length()), 0 };
