...
bool HostSignalInputThread::_AdvanceReader(DWORD byteCount)
{
    ...
    while (byteCount > 0)
    {
        const auto advance = std::min(byteCount, til::narrow_cast<DWORD>(buffer.max_size()));
...
bool HostSignalInputThread::_GetData(std::span<std::byte> buffer)
{
    ...
    if (FALSE == ReadFile(_hFile.get(), buffer.data(), til::narrow_cast<DWORD>(buffer.size()), &bytesRead, nullptr))
    {
