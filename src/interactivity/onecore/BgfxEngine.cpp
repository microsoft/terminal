...
BgfxEngine::BgfxEngine(PVOID SharedViewBase, LONG DisplayHeight, LONG DisplayWidth, LONG FontWidth, LONG FontHeight) noexcept :
    ...
    _displayHeight(til::narrow_cast<SIZE_T>(DisplayHeight)),
    _displayWidth(til::narrow_cast<SIZE_T>(DisplayWidth)),
    _currentLegacyColorAttribute(DEFAULT_COLOR_ATTRIBUTE)
{
    _runLength = sizeof(CD_IO_CHARACTER) * DisplayWidth;

    _fontSize.width = FontWidth > SHORT_MAX ? SHORT_MAX : til::narrow_cast<til::CoordType>(FontWidth);
    _fontSize.height = FontHeight > SHORT_MAX ? SHORT_MAX : til::narrow_cast<til::CoordType>(FontHeight);
}
                                                  ...
                                                  const bool /*lineWrapped*/) noexcept
{
    try
    {
        const auto y = til::narrow_cast<SIZE_T>(coord.y);
        ...
        {
            NewRun[coord.x + i].Character = til::at_unchecked(clusters, i).GetTextAsSingle();
...
try
{
    ...
    CursorInfo.Row = wil::safe_cast<USHORT>(options.coordCursor.y);
    CursorInfo.Column = wil::safe_cast<USHORT>(options.coordCursor.x);
...
{
    _dirtyArea.bottom = til::narrow_cast<til::CoordType>(std::max(static_cast<SIZE_T>(0), _displayHeight));
    _dirtyArea.right = til::narrow_cast<til::CoordType>(std::max(static_cast<SIZE_T>(0), _displayWidth));
