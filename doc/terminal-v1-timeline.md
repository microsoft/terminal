...
| V1 | 0 | Code Reuse | Terminal's core engine will re-use & share componentry from within Windows Console wherever feasible to minimize support & maintenance costs for both|
...
| V1 | 0 | Rich, modern text renderer | Terminal must be able to render glyphs from East Asian and Middle Asian languages, inc. Chinese, Hebrew, Arabic, etc. Terminal will also be able to render Emoji - an increasingly important feature consdiering that several programming languages now support Emoji in method and variable names! To render such glyphs, the Terminal needs a DirectWrite-based layout & rendering system which supports font-fallback, customizable text-layout, GPU accelerated rendering, and many other features not currently supported by the built-in Windows Console |
...
| V1 | 0 | Elevation | Terminal can be launched "elevated" with Admin rights if required so that the user can perform operatoins that affect machine-wide state |
...
| V1 | 1 | Customizable Key Bindings | Terminal will provide a way for users to customize key bindings, enabling them to configure speccific key-chorts to particular Terminal actions |
